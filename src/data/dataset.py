...
from src.data.database import BaseDataset
...
class Cityscapes(BaseDataset):
    ...
    def __init__(self,
        ...
        self.root = root
        if is_train:
            self.list_path = root + "/mindtrain.txt"
        else:
            self.list_path = root + "/mindval.txt"
...
def multi_scale_inference(dataset, model, image, scales=None, flip=False):
    ...
    stride_h = np.int(dataset.crop_size[0] * 1.0)
    stride_w = np.int(dataset.crop_size[1] * 1.0)

    final_pred = Tensor(np.zeros([1, dataset.num_classes, ori_height, ori_width]), dtype=dtype.float32)
    for scale in scales:
        new_img = dataset.multi_scale_aug(image=image,
                                          rand_scale=scale,
                                          rand_crop=False)
        ...
        if scale <= 1.0:
            ...
            preds = dataset.inference(model, new_img, flip)
            preds = preds.asnumpy()
            preds = preds[:, :, 0:height, 0:width]
        else:
            new_h, new_w = new_img.shape[:-1]
            rows = np.int(np.ceil(1.0 * (new_h -
                                         dataset.crop_size[0]) / stride_h)) + 1
            cols = np.int(np.ceil(1.0 * (new_w -
                                         dataset.crop_size[1]) / stride_w)) + 1
            preds = np.zeros([1, dataset.num_classes, new_h, new_w]).astype(np.float32)
            count = np.zeros([1, 1, new_h, new_w]).astype(np.float32)
            for r in range(rows):
                for c in range(cols):
                    h0 = r * stride_h
                    w0 = c * stride_w
                    h1 = min(h0 + dataset.crop_size[0], new_h)
                    w1 = min(w0 + dataset.crop_size[1], new_w)
                    h0 = max(int(h1 - dataset.crop_size[0]), 0)
                    w0 = max(int(w1 - dataset.crop_size[1]), 0)
